...
HAAR_CASCADE_DIR = os.path.join(BASE_DIR, "data")
...
def download_haarcascade(url: str, save_path: str):
    """
    Downloads the Haar cascade XML file if it doesn't already exist.
    """
    if os.path.exists(save_path):
        Logger(f"[INFO] Haar cascade file already exists at {save_path}.")
        return

    Logger(f"[INFO] Haar cascade file not found. Downloading from {url}...")
    try:
        ...
        Logger(f"[INFO] Successfully downloaded Haar cascade file to {save_path}.")
    except requests.exceptions.RequestException as e:
        Logger(f"[ERROR] Failed to download Haar cascade file: {e}")
        raise RuntimeError("Could not download Haar cascade classifier.")
...
class FaceAppBackend:
    def __init__(self, ui_instance=None):
        ...
        # Ensure Haar cascade file exists before loading
        try:
            download_haarcascade(HAAR_CASCADE_URL, HAAR_CASCADE_PATH)
            self.face_cascade = cv2.CascadeClassifier(HAAR_CASCADE_PATH)
            if self.face_cascade.empty():
                raise RuntimeError("Cannot load Haar cascade classifier after download.")
            Logger(f"[INFO] Successfully loaded Haar cascade from {HAAR_CASCADE_PATH}.")
        except Exception as e:
            Logger(f"[ERROR] Critical error during Haar cascade setup: {e}")
            self.face_cascade = None # Set to None to prevent crashes
